    ...
    try:
      ...
      if acesso:
        ...
        if dados is not None:
          ...
          for numero in range(15): # type: ignore
        ...
        sl.markdown(f'<h6 style ="color: #808080; ">Digite um número de 1 a {max}</h6>', unsafe_allow_html=True)
    ...
    if sl.button('Verificar') and inteiro == True and (0 < n <= dados['channel']['last_entry_id']): # type: ignore
      ...
      if dados is not None:
        ...
        for numero in range(n): # type: ignore
          ...
          horario.append(f'{dia}/{mes}/{ano} {hora}')
          horarioMin.append(f'{mes}/{dia} {hora}') #{dia}/{mes}
    ...
    if sl.button('Consultar'):
      if req5.status_code == 200:
        ...
        for n in range(len(info['list'])):
          ...
          <h5 style="height: 0rem;">Umidade: {umidade*100}%</h5>
          <h5 style="height: 0rem;">Probabilidade de Precipitação: {probabilidadeDeChuva*100}%</h5>
