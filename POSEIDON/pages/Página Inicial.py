...
if paginaSelecionada == 'Verificação':
  def interface():
    ...
    # teste condicional de funcionamento
      if volumes is not None:
        ...
        with tabGrafico:
          ...
          sl.markdown(
              f'''<h6 style="height: 3rem; color: #808080; ">Média: <span style="height: 0rem; color: {hex}">{Media} cm de volume ocupado</span></h6>''', unsafe_allow_html=True)  # (arredondamento da soma dos valores / quantidade de valores) - 19(distância onde começa a contagem (mínima))
        ...
        with tabBueiros:
          ...
          sl.markdown(
            f'''<h4 style="height: 3rem; color: #7c7c7c;">Prioridade atual dos sistemas Poseid.on:</h4>
            <h6 style="height: 2rem;">{atual[str(decrescente[0])]} em <span style="height: 0rem; color: {hex4}">{decrescente[3]}cm</span> de volume ocupado</h6>
            <h6 style="height: 2rem;">{atual[str(decrescente[1])]} em <span style="height: 0rem; color: {hex3}">{decrescente[2]}cm</span> de volume ocupado</h6>
    ...
    url5dias = f'https://api.openweathermap.org/data/2.5/forecast?q={cidade},{codigoDoPais}&appid=4127401294a510735af86031ebc9697b&units=metric&lang={"pt_br"}'
